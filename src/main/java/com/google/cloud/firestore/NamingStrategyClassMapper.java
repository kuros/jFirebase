...
public class NamingStrategyClassMapper extends CustomClassMapper {
    ...
    @SuppressWarnings("unchecked")
    private static <T> Object serialize(T o, NamingStrategyClassMapper.ErrorPath path) {
        ...
        if (o == null) {
            ...
            try {
                ...
                return NamingStrategyClassMapper.BeanMapper.propertyName(enumField);
            ...
            NamingStrategyClassMapper.BeanMapper<T> mapper = loadOrCreateBeanMapperForClass(clazz);
    ...
    private static <T> T deserializeToType(Object o, Type type,
                                           NamingStrategyClassMapper.DeserializeContext context) {
        if (o == null) {
            return null;
    ...
    private static <T> T deserializeToParameterizedType(
            Object o, ParameterizedType type, NamingStrategyClassMapper.DeserializeContext context) {
        ...
        if (List.class.isAssignableFrom(rawType)) {
            ...
            Map<String, Object> map = expectMap(o, context);
            NamingStrategyClassMapper.BeanMapper<T>
                    mapper =
                    (NamingStrategyClassMapper.BeanMapper<T>) loadOrCreateBeanMapperForClass(rawType);
    ...
    private static <T> T deserializeToPrimitive(
            Object o, Class<T> clazz, NamingStrategyClassMapper.DeserializeContext context) {
    ...
    private static <T> T deserializeToEnum(
            Object object, Class<T> clazz, NamingStrategyClassMapper.DeserializeContext context) {
        if (object instanceof String) {
            ...
            for (Field field : enumFields) {
                if (field.isEnumConstant()) {
                    String propertyName = NamingStrategyClassMapper.BeanMapper.propertyName(field);
    ...
    private static <T> NamingStrategyClassMapper.BeanMapper<T> loadOrCreateBeanMapperForClass(
            Class<T> clazz) {
        @SuppressWarnings("unchecked")
        NamingStrategyClassMapper.BeanMapper<T>
                mapper = (NamingStrategyClassMapper.BeanMapper<T>) mappers.get(clazz);
        if (mapper == null) {
            mapper = new NamingStrategyClassMapper.BeanMapper<>(clazz);
    ...
    private static Long convertLong(Object o, NamingStrategyClassMapper.DeserializeContext context) {
        if (o instanceof Integer) {
    ...
    private static Date convertDate(Object o, NamingStrategyClassMapper.DeserializeContext context) {
    ...
    private static Timestamp convertTimestamp(Object o,
                                              NamingStrategyClassMapper.DeserializeContext context) {
    ...
    private static Blob convertBlob(Object o, NamingStrategyClassMapper.DeserializeContext context) {
    ...
    private static IllegalArgumentException serializeError(
            NamingStrategyClassMapper.ErrorPath path, String reason) {
    ...
    private static class BeanMapper<T> {
        ...
        private void addProperties(Class<T> clazz) {
            ...
            for (Method method : clazz.getMethods()) {
                if (shouldIncludeGetter(method)) {
                    ...
                    // This prevent using inheritance
...
//                    }
        ...
        private void addSetters(Class<T> clazz) {
        ...
        T deserialize(Map<String, Object> values,
                      NamingStrategyClassMapper.DeserializeContext context) {
        ...
        T deserialize(
            ...
            for (Map.Entry<String, Object> entry : values.entrySet()) {
                ...
                if (setters.containsKey(propertyName)) {
                    ...
                    Object value =
                            NamingStrategyClassMapper.deserializeToType(
        ...
        private T getInstance(Map<String, Object> values) {
            Optional<JsonSubTypes.Type> foundSubType = findSubType(values);

            T instance;
            try {
                if (foundSubType.isPresent()) {
                    Class foundClazz = foundSubType.get().value();
                    addProperties(foundClazz);
                    addSetters(foundClazz);
                    Constructor subTypeConstructor = foundClazz.getDeclaredConstructor();
                    subTypeConstructor.setAccessible(true);
                    instance = (T) subTypeConstructor.newInstance();
                } else {
        ...
        private Optional<JsonSubTypes.Type> findSubType(Map<String, Object> values) {
            // TODO add support for other type info,
            //  currently support only property
            JsonTypeInfo jsonSubType = clazz.getAnnotation(JsonTypeInfo.class);
            String property = Objects.isNull(jsonSubType) ? null : jsonSubType.property();
            Optional<JsonSubTypes.Type> foundSubType = Optional.empty();
            if (!TextUtils.isEmpty(property)) {
                JsonSubTypes subTypes = clazz.getAnnotation(JsonSubTypes.class);
                JsonSubTypes.Type[] childTypes = subTypes.value();
                for (Map.Entry<String, Object> entry : values.entrySet()) {
                    if (entry.getKey().equals(property)) {
                        String typeValue = (String) entry.getValue();
                        foundSubType = Arrays.stream(childTypes).filter(type -> type.name().equals(typeValue)).findFirst();
                    }
                }
            }
            return foundSubType;
        ...
        Map<String, Object> serialize(T object, NamingStrategyClassMapper.ErrorPath path) {
            ...
            for (String property : properties.values()) {
                ...
                if (serverTimestamps.contains(property) && propertyValue == null) {
                    ...
                    serializedValue =
                            NamingStrategyClassMapper.serialize(propertyValue, path.child(property));
                }
    ...
    static class ErrorPath {
        ...
        private final NamingStrategyClassMapper.ErrorPath parent;
        private final String name;

        static final NamingStrategyClassMapper.ErrorPath
                EMPTY = new NamingStrategyClassMapper.ErrorPath(null, null, 0);

        ErrorPath(NamingStrategyClassMapper.ErrorPath parent, String name, int length) {
            this.parent = parent;
            this.name = name;
            this.length = length;
        }
        ...
        NamingStrategyClassMapper.ErrorPath child(String name) {
            return new NamingStrategyClassMapper.ErrorPath(this, name, length + 1);
        }
    ...
    private static class DeserializeContext {
         ...
         */
        final NamingStrategyClassMapper.ErrorPath errorPath;
        ...
        DeserializeContext(NamingStrategyClassMapper.ErrorPath path, DocumentReference docRef) {
            errorPath = path;
            documentRef = docRef;
        }

        NamingStrategyClassMapper.DeserializeContext newInstanceWithErrorPath(
                NamingStrategyClassMapper.ErrorPath newPath) {
            return new NamingStrategyClassMapper.DeserializeContext(newPath, documentRef);
        }
    }
}
